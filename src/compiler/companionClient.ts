    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          return this.status("unavailable", "The Typr Server API key is missing or was rejected.");
        }
...
export function validateCompanionApiKey(value: string): CompanionApiKeyValidation {
  ...
  if (!/^typr_[A-Za-z0-9_-]{43}$/.test(trimmed)) {
    return {
      ok: false,
      message: `The Typr Server API key must be the complete ${COMPANION_API_KEY_PREFIX} key shown when it was created.`
    };
  }
