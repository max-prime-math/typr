...
This is the durable execution record for splitting Typr Server into its own
...
- Move Typr Server to the public `max-prime-math/typr-server` repository
...
- Keep the existing image identity `ghcr.io/max-prime-math/typr-server` and
  release Typr Server `v0.1.2` only after its extracted repository is validated.
- Make Typr consume a reproducibly pinned copy of the Typr Server protocol owned
  by the Typr Server repository.
...
- Keep IndexedDB/browser-local projects as the default. A mapped Typr Server
...
- Publish Typr and Typr Server independently to GHCR, with optional Docker
...
- Existing Typr Server image name: `ghcr.io/max-prime-math/typr-server`.
...
performed from `dev`, where all existing Typr Server release work lives. It must
...
- Typr Server was introduced by `52c5647` together with the frontend client and
...
- The extracted repository owns `typr-server/**`, the Typr Server Dockerfile and
  patch/runtime manifests, four Docker harness scripts, `tsconfig.server.json`,
  Typr Server Compose files, Typr Server workflow/docs/examples, and the Typr Server
...
- Frontend Typr Server clients, preview UI, settings, E2E coverage, and user guide
  remain in Typr. The current `src/companion-protocol/**` is the co-owned split
  blocker and becomes Typr Server-owned before Typr deletes its copy.
- The old Typr Server workflow must be removed from Typr before a future Typr
...
- IndexedDB database `typr` is already the authoritative startup/autosave store;
  OPFS is a browser-private mirror and local-folder sync is explicit. Typr Server
...
- There is no Typr web image today. The current `.dockerignore`, Compose files,
  and Docker workflow are Typr Server-only and must be replaced or moved during
  the split.
- Plain LAN HTTP is not a secure context (except browser loopback special cases),
  ...
  also cannot call an HTTP Typr Server because of mixed-content rules. These are
...
- Existing Typr Server docs are strong on LAN/VPN and mixed-content constraints,
...
Typr Server v0.1.1 architecture. Amend this section before implementation if a
repository audit reveals a conflict.

- History split: build a filtered clone from Typr `dev`, retaining only
  Typr Server-owned paths and their relevant commits. Rename paths into a clean
  standalone layout in the filtered clone. Do not run history rewriting in the
  original Typr checkout.
- Shared protocol: the Typr Server repository is authoritative. Typr consumes its
...
- Releases: Typr Server and Typr have independent workflows, versions, tags,
...
### Stage 1 — Standalone Typr Server repository and protocol boundary
...
- [x] Pin the Typr Server-owned protocol by immutable commit archive and lock
  integrity.
- [x] Switch stable frontend and experimental TeXpresso imports to the package
  exports.
- [x] Remove Typr Server build/release/Compose/Unraid/docs ownership from
...
### Stage 3 — Optional mapped Typr Server workspace
...
- [x] Keep browser IndexedDB authoritative and make even Typr Server status
  polling opt-in: a clean self-hosted profile performs no cross-origin request
  until a user explicitly applies a Typr Server URL.
...
Status: complete (`54ccebe` Typr; `87f53e4` Typr Server; final state record in
...
- [x] Integrate the reviewed Typr Server Compose workspace override, strengthened
  Typr Server Unraid template/profile, documentation, workflow checks, and smoke
...
Status: complete (`46b18a5` + `a9d347a` Typr; `3ba7417` Typr Server)

- [x] Make Typr Server publish only `typr-server` from a fresh annotated
...
- [x] Give Typr a distinct `typr-vMAJOR.MINOR.PATCH` release grammar so the
  historical Typr Server `v0.1.0`/`v0.1.1` tags are never moved or reinterpreted.
...
Status: complete (`f6bbc40` Typr; `9290941` Typr Server)
...
- [x] Commit and push every verified Typr Server repository milestone.
...
- [x] Create and push one annotated Typr Server `v0.1.2` tag from the tested
  commit without moving either historical Typr tag.
- [x] Monitor Typr Server publication and verify public anonymous pulls by
  digest/platform.
...
- [x] Publish the compatible stateless-Unraid fallback as a new Typr Server
...
- [x] Install separate Typr and Typr Server user templates on Uma
...
- [x] Establish that Uma's stock kernel does not implement Landlock inside the
  container. Seccomp changes do not provide it, so a mapped Typr Server workspace
...
- [x] Publish Typr Server `v0.1.3`, install the matching updated user template,
...
- 2026-08-09: filtered `dev` into seven Typr Server-relevant commits with no tag
...
- 2026-08-10: standalone Typr Server commits `f5ddaca` through `44d0b7d` added
...
- 2026-08-10: all 8 Typr Server unit files / 57 tests, typecheck, production and
...
- 2026-08-10: Typr pins the Typr Server protocol archive at immutable commit
...
- 2026-08-10: the reviewed Typr Server Stage 5 patch was integrated byte-for-byte
...
- 2026-08-10: Stage 6 added independent, fail-closed release workflows at Typr
  `46b18a554cc73623fbb9f3f29d6430dce31d6174` and Typr Server
...
- 2026-08-10: dependency remediation reduced Typr and Typr Server `npm audit` to
  ...
  Playwright 1.62.1, jsPDF 4.2.1, PDF.js 6.2.108, and SVG-Edit 7.4.2. Typr Server
...
- 2026-08-10: the exact Typr Server image identifies
...
- 2026-08-10: Typr Server branch workflow `31390474399` passed at
...
- 2026-08-10: Typr Server release recovery run `31402441361` revalidated the
...
- 2026-08-10: workflow digest verification was fixed to consume complete
  Buildx output under `pipefail` (`abba7cf` Typr; `68e58c0` Typr Server).
...
- 2026-08-10: fresh annotated frontend tag `typr-v0.1.0` remains fixed at
  `828c4cad67fcb3a071f6d688f647bb0f566202d1`; historical Typr Server tags
...
- 2026-08-10: Uma returned `Function not implemented` from the production
  Landlock probe. Typr Server commits `c1f2e10` and `dffaeab` added the approved
  ...
  startup as designed. The installed Typr Server user-template SHA-256 is
  `1a909ca4d533663ce7b57829bf38aa3bd69487c3a285c964c47ec7f09a9136db`.
- 2026-08-10: Typr Server branch workflows `31415211246` and `31415825840`
...
The split, Typr v0.1.0 full/lite GHCR release, Typr Server v0.1.2 release, and
Typr Server v0.1.3 stateless-Unraid compatibility release are complete. Both user
...
client-trusted private HTTPS/reverse-proxy test (including Typr Server WebSockets),
