...
Production builds can enable an optional in-app sign-in layer with `VITE_TYPR_AUTH_USERS_SHA256`. Google Drive project sync is enabled by setting `VITE_GOOGLE_DRIVE_CLIENT_ID` to a Google OAuth web client whose authorized JavaScript origins include the deployment origin and whose authorized redirect URIs include the deployment root, such as `https://typr.ca/`. The Google Drive API must be enabled for that Google Cloud project. See [Architecture Overview](docs/architecture/overview.md) and the source in [src/auth/AuthGate.tsx](src/auth/AuthGate.tsx) for implementation context.
